...
def optimize_m(t_ini, f_ini,alpha_ini, sig_curr):
    ...
    res = scipy.optimize.minimize(Agrad.value_and_grad(lambda tt: -1.*lnpost(tt,f_ini,alpha_ini,sig_curr)),
                                  t_ini, # theta initial
                                  jac=True, 
                                  method='L-BFGS-B', 
...
def optimize_fa(t_ini, f_ini,alpha_ini, sig_curr):
    #keeping in mind that minimize requires flattened arrays
    res = scipy.optimize.minimize(Agrad.value_and_grad(lambda ff,aa: -1.*lnpost(t_ini,ff,aa,sig_curr)),
                                  t_ini, # theta initial
                                  jac=True, 
                                  method='L-BFGS-B', 
                                  bounds=[(1e-5, 10)]*2)
