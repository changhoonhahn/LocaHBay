...
def hess_k(ws,fdensity,alpha,sig,psf_k):
    ...
    ws = fft.ifft2(ws);
    #calc l1
    l1 = psf_k**2 /sig_noise**2 / n_grid**2;
    #calc l2
    xsi = (1.-fdensity ) * gaussian(np.log(ws),loc=np.log(mo), scale=sig)/ws + fdensity*(ws**alpha /w_norm)
...
def grad_k(ws,fdensity,alpha,sig,psf_k):
    ...
    ws = ws.reshape((n_grid,n_grid));
    ws = fft.ifft2(ws);
    l1 = fft.ifft2((Psi(ws) - data)).flatten()*psf_k/sig_noise**2;
...
def optimize_m(t_ini, f_ini,alpha_ini, sig_curr,psf_k):
    ...
    '''     
    res = scipy.optimize.newton(lambda tt: np.zeros(n_grid**2)-1*lnpost_k(tt,f_ini,alpha_ini,sig_curr),t_ini,fprime=grad_fun,fprime2=hess_fun);
    tt_prime = fft.ifft(res);
    print(tt_prime);
    '''                              
    tt_prime = res['x'];
...
tt_prime = optimize_m(tt0,f_curr,a_curr,sig_delta,psf_k);
