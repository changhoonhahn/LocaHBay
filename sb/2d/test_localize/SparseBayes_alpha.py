...
class SparseBayes_alpha:
    ...
    def __init__(self,data,psf,psf_k,no_source):
        ...
        self.wlim = (0.01,5); #min and max signal (given by challenge, will determine ourselves in the future)
    ...
    def aalpha(self,th,xi,f,alpha):
        gfunc = Agrad.grad(lambda aa: self.loss_fn(th,xi,f,aa));
        grad = gfunc(alpha);
        return grad;
    ...
    def optimize_alpha(self,wsp_k,xi,f,alpha):
        print('optimizing')
        gradfun = lambda aa: self.aalpha(wsp_k,xi,f,aa);
        res = scipy.optimize.minimize(lambda at: self.loss_fn(wsp_k,xi,f,at),
            alpha, # theta initial
            jac=gradfun,                          
            method='Newton-CG');
        print('Number of Iterations alpha');
        print(res['nit']);
        return res['x'];
